...
class PlaneView extends egret.Sprite {
    ...
    public setView(gridId) {
        ...
        let gameView = SceneManager.getInstance().placingScene.gameView;
        let gridView = gameView.gridList[gridId] as PlacingGridView;
        ...
        for (let bodyType = 0; bodyType < bodyGrids.length; bodyType++) {
            this.addGrid(direction, "_" + bodyType, gridId, (gameView.gridList[headGridId + bodyGrids[bodyType]] as PlacingGridView).id);
        }
    }
